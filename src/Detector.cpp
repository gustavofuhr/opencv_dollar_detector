	...
	for( int c=0; c<lastCol; c++ ) 
	{
		for( int r=0; r<lastRow; r++ ) 
		{
			...
			if (boundingBoxWorldHeight >= minPedestrianHeight)
			{
				...
				int scaleIndex = findBestScale(boundingBoxWorldHeight, minPedestrianHeight, maxPedestrianHeight, opts.pPyramid.scales);
				int height = pyramid[scaleIndex].image.rows;
				int width = pyramid[scaleIndex].image.cols;
				float* chns = (float*)malloc(height*width*channels*sizeof(float));
				features2floatArray(pyramid[scaleIndex], chns, height, width,  opts.pPyramid.pChns.pColor.nChannels, opts.pPyramid.pChns.pGradMag.nChannels, opts.pPyramid.pChns.pGradHist.nChannels);

				float scale_x = pyramid[scaleIndex].image.cols / pyramid[0].image.cols;
				float scale_y = pyramid[scaleIndex].image.rows / pyramid[0].image.rows;

				cv::Mat scaledHomography = scaleHomographyMatrix(homography, scale_x, scale_y);
				...
				// construct cids array
			  	...
			  	for( int z=0; z<nChns; z++ )
			    	for( int col=0; col<modelWd/shrink; col++ )
			      		for( int row=0; row<modelHt/shrink; row++ )
			        		cids[m++] = z*width*height + col*height + row;

			    float h=0, *chns1=chns+(scaledR*stride/shrink) + (scaledC*stride/shrink)*height;
			    if( treeDepth==1 ) {
				...
				if(h>cascThr) { cs.push_back(scaledC); rs.push_back(scaledR); hs1.push_back(h); }
				...
				for(int j=0; j<m; j++ )
				{
					...
					bb.topLeftPoint.x = (bb.topLeftPoint.x+shift[1])/opts.pPyramid.scales_w[scaleIndex];
					bb.topLeftPoint.y = rs[j]*stride;
					bb.topLeftPoint.y = (bb.topLeftPoint.y+shift[0])/opts.pPyramid.scales_h[scaleIndex];
					bb.height = opts.modelDs[0]/opts.pPyramid.scales[scaleIndex];
					bb.width = opts.modelDs[1]/opts.pPyramid.scales[scaleIndex];
					bb.score = hs1[j];
					bb.scale = scaleIndex;
					result.push_back(bb);
				}
