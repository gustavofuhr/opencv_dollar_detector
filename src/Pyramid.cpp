...
std::vector<Info> Pyramid::computeMultiScaleChannelFeaturePyramidSmart(cv::Mat I, int *modelDimensions, double candidateMaxHeight)
{
	int colorChannels = pChns.pColor.nChannels;
	int histogramChannels = pChns.pGradHist.nChannels;
	clock_t start, end;
	...
	*/
	...
	// get scales at which to compute features and list of real/approx scales
	// [scales,scaleshw]=getScales(nPerOct,nOctUp,minDs,shrink,sz);
	getScalesSmart(I.rows, I.cols, pChns.shrink, modelDimensions, candidateMaxHeight);
	std::cout << "Number of computed scales: " << computedScales << std::endl;
...
void Pyramid::getScalesSmart(int h, int w, int shrink, int *modelDimensions, int maxCandidateHeight)
{
	...
	if (h!=0 && w!=0)
	{
		...
		for (int i=0; i < computedScales; i++)
		{
			...
			validScales++;
			if (modelSize > maxCandidateHeight) break; 
		}
		

		computedScales = validScales;
